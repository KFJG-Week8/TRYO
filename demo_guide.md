...
터미널 A에서는 서버를 빌드하고, 이전 시연 데이터를 지운 뒤, 서버를 실행합니다.
...
터미널 A는 서버가 계속 떠 있어야 하므로 그대로 둡니다.

터미널 B에서는 서버 상태를 확인하고, 발표자용 자동화 스크립트를 실행합니다.
...
`scripts/demo_presenter.sh`가 실행되면 각 단계마다 명령어를 먼저 보여줍니다.
Enter를 누르면 해당 명령어가 실행되고, `s`를 입력한 뒤 Enter를 누르면 그 단계는 건너뜁니다.
